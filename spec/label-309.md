...
- **Content hash** — a cryptographic digest of the content under a named hash algorithm from the hash registry [`../registries/hash-algorithms.json`](../registries/hash-algorithms.json); the primary claim of a PoE record.
...
[`../cddl/label-309.cddl`](../cddl/label-309.cddl); per-component JSON Schemas in
[`../schemas/`](../schemas) mirror it for tooling. Algorithm identifiers are
drawn from the registries in [`../registries/`](../registries). Conformance
vectors are pinned under [`../conformance/`](../conformance). Each artefact is
...
([`../registries/companion-cips.json`](../registries/companion-cips.json),
...
(CBOR text strings from [`../registries/hash-algorithms.json`](../registries/hash-algorithms.json))
...
CBOR map invariant in [`../cddl/label-309.cddl`](../cddl/label-309.cddl)). A
...
all 256-bit hashes in [`../registries/hash-algorithms.json`](../registries/hash-algorithms.json),
...
| ------------ | ------------------- | -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `alg`        | tstr                | REQUIRED | Registered list-commitment algorithm identifier (see [`../registries/merkle-commitment-algorithms.json`](../registries/merkle-commitment-algorithms.json)) |
...
per-leaf inclusion proofs) live under [`../conformance/`](../conformance).
...
Signers and verifiers **MUST** agree on this canonicalisation bit-for-bit; implementations **SHOULD** use a CBOR library that supports RFC 8949 deterministic encoding natively. The complete grammar for the canonical record body is the CDDL at [`../cddl/label-309.cddl`](../cddl/label-309.cddl); conformance vectors that pin exact canonical bytes are at [`../conformance/`](../conformance).
...
| **Multihash**                | `0x12` (sha2-256, 32-byte digest), `0xb220` (blake2b-256, 32-byte digest)                       | Mirrors the content-hash registry ([`../registries/hash-algorithms.json`](../registries/hash-algorithms.json)), so an implementer can reuse the same primitives. Other codes **MUST** be rejected. |
...
[`../registries/signature-algorithms.json`](../registries/signature-algorithms.json).
...
| Validity      | Any 32-byte value is a valid input. Implementations MUST NOT reject a seed for low-entropy patterns at the derivation API; the all-zero seed is a valid input and is used by the conformance vectors (see [`../conformance/`](../conformance)) for byte-exact reproducibility |
...
[Sealed PoE: multi-recipient encryption](#sealed-poe-multi-recipient-encryption);
its registry entry is [`../registries/kem-algorithms.json`](../registries/kem-algorithms.json).
...
KEM registry [`../registries/kem-algorithms.json`](../registries/kem-algorithms.json).
...
machine artifact [`../cddl/label-309.cddl`](../cddl/label-309.cddl); the cross-field
...
[`../registries/kem-algorithms.json`](../registries/kem-algorithms.json); the
content format is [`../registries/aead-algorithms.json`](../registries/aead-algorithms.json).
...
[`../registries/error-codes.json`](../registries/error-codes.json); a conformant
...
`argon2id` (see [`../registries/kdf-algorithms.json`](../registries/kdf-algorithms.json));
...
[`../conformance/sealed-poe/`](../conformance/sealed-poe). They are produced from
...
The normative machine-readable form of every registry lives under [`../registries/`](../registries):

| Registry                          | File                                                                                                 | Wire role                              |
| --------------------------------- | ---------------------------------------------------------------------------------------------------- | -------------------------------------- |
| Hash algorithms                   | [`../registries/hash-algorithms.json`](../registries/hash-algorithms.json)                           | `items[i].hashes` map keys             |
| Merkle list-commitment algorithms | [`../registries/merkle-commitment-algorithms.json`](../registries/merkle-commitment-algorithms.json) | `merkle[i].alg`                        |
| AEAD algorithms                   | [`../registries/aead-algorithms.json`](../registries/aead-algorithms.json)                           | `enc.aead`                             |
| KEM algorithms                    | [`../registries/kem-algorithms.json`](../registries/kem-algorithms.json)                             | `enc.kem`                              |
| KDF algorithms                    | [`../registries/kdf-algorithms.json`](../registries/kdf-algorithms.json)                             | `enc.passphrase.alg`                   |
| Signature algorithms              | [`../registries/signature-algorithms.json`](../registries/signature-algorithms.json)                 | `COSE_Sign1` protected `alg`           |
| Error codes                       | [`../registries/error-codes.json`](../registries/error-codes.json)                                   | structural-validator / verifier output |
...
and the pinned conformance vectors under [`../conformance/`](../conformance) are
...
2. Add the identifier to the appropriate registry under [`../registries/`](../registries) with its pinned length constant and the typed error code emitted on each failure mode.
3. Ship a byte-pinned conformance vector under [`../conformance/`](../conformance) that exercises the new identifier.
...
The authoritative error catalogue is the machine-readable registry [`../registries/error-codes.json`](../registries/error-codes.json); the conformance vectors that pin each failure mode to its code live under [`../conformance/`](../conformance) (validator-rejection cases in [`../conformance/validator/`](../conformance/validator), cross-service / service-independence cases in [`../conformance/cross-service/`](../conformance/cross-service)). A conformant implementation MUST emit exactly the `SCREAMING_SNAKE_CASE` codes defined there for the failure modes defined there; it MUST NOT introduce lowercase synonyms, `schema_*`-prefixed parser-internal codes, or free-form reason strings. The families summarized below are the index into that registry, not a substitute for it.
...
Every code below is emitted by `validatePoeRecord` and carries `severity: error` unless noted. The registry [`../registries/error-codes.json`](../registries/error-codes.json) is authoritative for the complete list and the exact trigger of each code.
...
The verifier-layer codes are never emitted by the structural validator; they appear only in the verifier's report. The registry [`../registries/error-codes.json`](../registries/error-codes.json) is authoritative.
...
[`../conformance/`](../conformance), not the Internet-Draft, are authoritative
...
`blake2b-256` (registered in [`../registries/hash-algorithms.json`](../registries/hash-algorithms.json),
...
[`../registries/merkle-commitment-algorithms.json`](../registries/merkle-commitment-algorithms.json))
...
[`../registries/error-codes.json`](../registries/error-codes.json)) — with no
...
1. **Reference implementations exist and pass conformance.** At least one
   ...
   [`../examples/`](../examples); all consume the byte-pinned vectors in
   [`../conformance/`](../conformance).
2. **Conformance suite.** A public conformance suite exists — valid and invalid
   fixtures, with every error code in
   [`../registries/error-codes.json`](../registries/error-codes.json) covered by
...
1. **Conformance fixture set.** Publish the machine-readable fixtures under
   [`../conformance/`](../conformance) and a runner that maps every error code in
   [`../registries/error-codes.json`](../registries/error-codes.json) to at least
...
2. **Reference SDKs.** Publish a [TypeScript SDK](https://github.com/cardanowall/label-309-ts),
   ...
   the reference implementations live under [`../examples/`](../examples).
3. **Standalone public verifier and CLI.** Ship a
   [verifier-only CLI](https://github.com/cardanowall/label-309-cli) and a static
   web verifier. Both load from [`../conformance/`](../conformance) for unit tests
...
This CIP is licensed under [CC-BY-4.0](../LICENSE-docs).
